...
> **冻结日期：2026-06-02**
>
> 本文件仅作为历史上下文归档，记录早期执行线程曾使用的完整任务提示。**不要再按本 Prompt 开发、排期或拆分任务**，也不要把本文中的旧 30/50/20 模型、旧合约数量、旧执行顺序当作当前事实来源。
>
> 仍有价值的需求已经迁移到当前唯一执行计划文档：[`VIBECODER_PHASE_PLAN.md`](./VIBECODER_PHASE_PLAN.md)。后续执行线程必须只以 `VIBECODER_PHASE_PLAN.md` 为任务入口，并在需要产品/合约细节时按该文档索引引用 `CONTRACTS.md`、`VIBECODER_PLAN.md` 等来源。
