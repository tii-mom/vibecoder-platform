...
```
30% → 团队钱包           （55% 触发后立即释放）
44% - 48.5% → 治理合约   （团队申请 → 投票释放，随平台费动态配平，初始为 48.5%）
18% → 项目方              （自行支配）
3.5% - 8% → 平台 Fund     （平台服务费，动态调整，初始为 3.5%）
```
...
发布手续费   ：创作者发布任务时收取 1% - 5% 服务费（动态可调，前期为激励创作者设置为 **1%**，后期逐步调升至 5%；以赏金代币支付；若使用外链代币则收固定 2 TON 平台服务费），成功扣除后自动转入国库基金合约 (Fund)
