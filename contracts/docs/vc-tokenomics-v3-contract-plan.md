...
- 每位参与发射的用户，每投资 1 TON 可获得 **1,000 VC**。
- **单用户上限 100,000 VC**。
- 用户首次可领取 **20%**（立即释放）。
- 剩余 **80%** 分 **4 轮**价格解锁：

| 轮次 | 比例 | 累积 | 触发价 (price scale) |
| ---: | ---: | ---: | ---: |
...
- 价格标尺：`PRICE_SCALE = 1_000_000`，`$X` 存为 `X × 1,000,000`。
- 必须防重复领取（按 user address + campaign address 绑定）。
- 必须绑定 Spark / Launch 参与记录（由 LaunchCampaign / Worker / indexer 提供证明）。
